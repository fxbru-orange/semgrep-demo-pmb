...
// +--------------------------------------------------------------------------+
// $Id: print.php,v 1.139.4.9 2023/12/22 08:34:50 dgoron Exp $
...
global $opac_biblio_name, $opac_parse_html, $opac_url_base;
...
if ($action != "print_$lvl") {
    ...
    if (! $id_liste && ! $id_etagere && ! $current_search) {
        ...
        $script_selnoti .= "
            function getSelectedNotice() {
                const selectNotiNode = document.getElementById('select_noti');
                if (selectNotiNode && selectNotiNode.checked) {
    ...
    $output_final .= "
    	...
    	<input type='hidden' name='select_noti' id='select_noti' value='" . (($lvl == "search") ? intval($_SESSION["tab_result_current_page"]) : "") . "'/>";
