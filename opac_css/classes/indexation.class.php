...
// +-------------------------------------------------+
// $Id: indexation.class.php,v 1.40.4.2 2023/10/13 12:51:14 dgoron Exp $
...
// 								$translated_text = translation::get_translated_text($tab_row["subst_for_translation_".$trans_table_key], $trans_table, $nom_champ, $liste_mots, $cle);
	...
	protected function maj_queries($object_id, $datatype='all') {
		foreach($this->queries as $k=>$v) {
			if ($this->check_datatype($datatype, $k)) {
				if($object_id) {
					$query = str_replace("!!object_id!!",$object_id,$v['rqt']);
				} else {
					$query = $v['rqt'];
				}
	...
	protected function maj_authperso_link($object_id, $datatype='all') {
		if(count($this->tab_authperso_link) && $this->check_datatype($datatype, 'authperso_link')){
	...
	protected function add_callable_data_tab_insert($object_id, $infos, $values) {
		...
		for($j=0 ; $j<count($values) ; $j++) {
			if (is_array($values[$j])) {
				foreach ($values[$j] as $callback_lang => $callback_value) {
					$this->add_tab_field_insert($object_id, $infos, $order_fields, $callback_value, $callback_lang);
					
					$this->add_data_tab_insert($object_id, $infos, $callback_value, $order_fields);
				}
			} else {
