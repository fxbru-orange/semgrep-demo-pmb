...
// +-------------------------------------------------+
// $Id: lettre_PDF.class.php,v 1.17.2.2 2024/01/19 14:15:22 dgoron Exp $
...
class lettre_PDF {
	...
	protected function set_language($language) {
        ...
        static::set_language_parameters($language);
	...
	protected function restaure_language() {
	    ...
	    static::set_language_parameters($lang);
	}
	...
	public static function get_language_parameters($language) {
	    ...
	    if(!isset(static::$languages_parameters[$language])) {
	        $parameters = [];
	        //parametres traduits
	        $query = "SELECT type_param, sstype_param, trans_small_text FROM translation
	        ...
	        if($result && pmb_mysql_num_rows($result)) {
    	        while ($row = pmb_mysql_fetch_object($result)) {
    	            ...
    	            if (!isset($overload_global_parameters[$field])) {
    	                $parameters[$field] = $row->trans_small_text;
    	            }
    	        }
	        }
	...
	protected static function set_language_parameters($language) {
