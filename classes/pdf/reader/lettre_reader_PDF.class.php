...
// +-------------------------------------------------+
// $Id: lettre_reader_PDF.class.php,v 1.18.2.1 2024/01/12 15:34:15 tsamson Exp $
...
class lettre_reader_PDF extends lettre_PDF {
	...
	protected function get_expl_info($cb_doc) {
		if(empty($this->expl_info[$cb_doc])) {
			...
			if ($expl->expl_location) {
			    $location = new docs_location($expl->expl_location);
			    $expl->location_libelle = $location->get_translated_libelle();
			}
			if ($expl->expl_section) {
			    $section = new docs_section($expl->expl_section);
			    $expl->section_libelle = $section->get_translated_libelle();
			}
